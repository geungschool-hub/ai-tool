...
학생 행 [점수보정] 버튼→모달(5점수+정답수/푼수 편집). 변경분을 wk*·daily에도 동일 반영. 이력 `vocab/scoreLog`(규칙 추가). **이 작업 PC는 학교망이 RTDB '쓰기'를 막음**(읽기·호스팅·규칙배포는 됨) → 데이터 직접수정 불가, 보정은 **교사 기기에서 admin.html로**. 해당 학생(<학번>·basic) 어제 6회시행 1418점 → 1세션값 248점(c190/k10/cb18/sp30, 정답률19/20)으로 보정 예정(미적용).
