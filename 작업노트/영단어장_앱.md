# 영단어장 앱 (english-vocab / gongju7-vocab)

> 2026-08-18 통합: `영단어장_앱.md` + `영단어장_앱.md`를 합쳤다.
> **위쪽 = 실제로 돌아가는 것(과거형 기록) · 아래쪽 「미구현 기획」 = 아직 안 만든 것**이니 섞어 읽지 말 것.

배포: https://gongju7-vocab.web.app (Firebase `gongju7-vocab`, 관리자 `/admin.html`, 관리자 학번 20700)
폴더: `학생앱/english-vocab/` · 상세 이력은 PC 메모리 `project_english_vocab.md` 참조(이 노트는 USB 동반 요약).
...
# B. 미구현 기획 — "대마왕 챌린지" (★아직 안 만들었다)

> 2026-06-17 구상. **사용자가 더 고민 중 → 결정되면 구현.** 두 달 넘게 이 상태이므로
> 다시 꺼낼 때는 「지금도 이 방향이 맞는지」부터 물을 것.
