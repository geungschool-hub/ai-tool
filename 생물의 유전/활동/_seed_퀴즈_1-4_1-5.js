...
'use strict';
const { quiz14, quiz15 } = require('./_build_퀴즈기초_1-4_1-5.js');
...
(async () => {
  ...
  for (const [k, v] of Object.entries(before)) {
    console.log('  ' + k.padEnd(34) + ' subject=' + (v.subject || '(none)') +
                ' order=' + v.order + ' ' + (v.questions || []).length + '문항');
  }

  for (const q of [quiz14, quiz15]) {
    if (before[q.id]) throw new Error('★이미 같은 id 가 라이브에 있다: ' + q.id + ' — 덮어쓰지 않는다');
  }
  console.log('\n올릴 것: ' + quiz14.id + ' (order 80) · ' + quiz15.id + ' (order 90) — id 충돌 없음');
  ...
  const bk = await j(`${DB}/quizBackups.json?${A}`,
    { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ at: new Date().toISOString(), why: '1-4기초·1-5기초 추가 전',
