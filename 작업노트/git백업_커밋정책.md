...
- **백업 목적의 푸시는 필요 없다** — USB 는 `C:\Users\USER\USB-E-backup` 으로 **10분마다 자동 백업**된다
  (**`.git` 포함** — 2026-09-15 확인. 커밋 이력도 같이 산다). 자세한 것은 아래 「USB 10분 자동백업」 절.
  ★단 **이 백업은 이 PC(학교 공유 PC) 한 대에만 걸려 있다.** 다른 PC 에는 없다.
...
- **english-vocab 단어 데이터**(`basic_words.json`, `plus_words.json`)는 저작권상 git 미추적.
  실본 위치: ① Firebase RTDB(`vocab/levels/{id}/words`) ② USB 원본 ③ `C:\Users\USER\USB-E-backup`(10분 주기 자동백업)
...
## USB 10분 자동백업 — 무엇이 어디서 도는가 (2026-09-15 정리)

| 부품 | 위치 | 비고 |
|---|---|---|
| 작업 스케줄러 항목 `AItoolBackup` | 이 PC의 작업 스케줄러(`\`) | 2026-05-27 시작, **10분 반복**. 사용자 로그인 중에만 |
| 창 숨김 래퍼 | `C:\Users\USER\.claude\ai-tool-backup-hidden.vbs` | wscript 로 아래 ps1 을 콘솔 없이 실행 |
| 본체 | `C:\Users\USER\.claude\ai-tool-backup.ps1` | robocopy `/E`(삭제 동기화 없음 · `.git` 포함 · `node_modules`·`.next`·정션 제외) |
| 목적지 | `C:\Users\USER\USB-E-backup\` | ※이름의 E 는 옛 드라이브 문자. 폴더는 그대로 쓴다(9.8 GB 재복사 피함) |
| **살아 있는지 보는 곳** | `C:\Users\USER\USB-E-backup\_마지막백업.txt` | 실제로 복사한 시각·원본 드라이브·robocopy 코드 한 줄. **이 파일 시각이 10분 이상 옛날이면 멈춘 것** |

### ★2026-09-15 사고 — 두 달 동안 「성공」만 찍고 아무것도 안 했다
- 스크립트가 `$src = 'E:\'` 로 **드라이브 문자를 박아** 두었는데 USB 가 **F:** 로 잡히자 `Test-Path` 실패 → `exit 0`.
  스케줄러 이력엔 10분마다 결과 0(성공)이 찍히니 **겉으로는 멀쩡해 보였다.** 마지막 실제 복사 2026-07-21 19:04.
- 고침: **드라이브 문자로 찾지 않는다.** C: 가 아닌 드라이브 중 `AI tool\CLAUDE.md` 가 있는 것을 USB 로 본다.
  그리고 실제 복사 뒤에만 `_마지막백업.txt` 를 쓴다 — 「돌았다」가 아니라 「복사했다」의 증거.
- 같은 원인으로 **`_생기부\` 정션 허브 7개도 `E:\…` 를 가리켜 끊겨 있었다** → 드라이브 문자 없는
  **볼륨 GUID 경로**(`\\?\Volume{…}\AI tool\…`)로 다시 만들었다. 이제 문자가 바뀌어도 안 끊긴다.
  (정션을 새로 만들 땐 `mklink /J 링크 "$((Get-Volume -DriveLetter F).Path)AI tool\…"` 꼴로.)
- 교훈: **USB 경로를 드라이브 문자로 고정하지 말 것.** 스크립트는 표지 파일로 찾고, 정션은 볼륨 GUID 로.
  `E:\` 고정이 남은 곳은 `_보관/…/_archive` 의 끝난 스크립트뿐(안 고침).
