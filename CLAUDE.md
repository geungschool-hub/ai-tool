# 수업자료 제작 프로젝트

## 작업 노트 (USB 동반 메모리) ★먼저 확인
- 위치: `작업노트/` 폴더 (이 USB 폴더와 함께 이동 → 어느 PC에서든 읽힘)
- **관련 작업을 시작하기 전에 해당 노트를 먼저 읽을 것.** PC에 묶인 자동 메모리와 달리 USB로 옮겨도 유지됨.
- 새로 기억할 내용이 생기면 `작업노트/`에 .md로 추가하고 `작업노트/README.md`에 한 줄 등록.
### ★진입점은 3개뿐 (이 순서대로)
1. **`작업노트/_재개지점.md`** — 지금 뭘 하다 말았는가. **'다음 할 일'은 오직 이 파일에만 있다**(다른 노트는 과거형 기록만).
2. **`작업노트/세특_재료지도.md`** — 세특 재료가 뭐가 어디에 몇 명분 있고 어디까지 흘렀는가. 명단·점수 정본도 여기서 파생.
3. **`작업노트/README.md`** — **노트 목록 정본**. 어떤 노트가 있는지는 여기서만 확인한다.
   갈래별(생물의 유전 / 세특·생기부 / 앱 / 부서 업무 / 방법·정책 / 이력)로 나뉘어 있고
   **🟢 지금 하는 일 · ⚪ 만들어 두고 쓰는 것 · 📦 끝난 기록 · 🔧 방법·정책** 표시가 붙어 있다.

> 새 노트를 만들면 `작업노트/README.md`에만 한 줄 등록할 것. **CLAUDE.md에는 목록을 복제하지 않는다**(2벌 관리하다 실제로 어긋났음).
> ★2026-08-18: 이 자리에 있던 19줄짜리 「노트 한 줄 요약」이 **36개 노트 중 절반만 담은 채 낡아 있어 지웠다** — 규칙대로 README 한 벌만 둔다.
> ★작업노트는 PUBLIC 저장소에 추적된다 — **학생 실명·학번을 노트 본문에 쓰지 말 것.** 실명 대조표는 gitignore된 데이터 폴더에 두고 노트엔 경로만.

## 사용자 정보
- 직업: 고등학교 교사 (2학년 담임)
- 담당 수업
  - **1학기: 생명과학**(일반선택, 2학년) — 한 학기 과목이라 **1학기로 끝났다**
  - **2학기: 생물의 유전**(진로선택, 2학년) — **지금 하고 있는 과목**. 강좌 B·C·D 71명(A 없음)
- 부서 업무: 학생맞춤통합지원, 위탁교육과정, 학력평가

## 2022 개정 과학과 교육과정 과목 구조

| 구분 | 과목 |
|------|------|
| 공통과목 | 통합과학1, 통합과학2, 과학탐구실험1, 과학탐구실험2 |
| 일반선택 | 생명과학 |
| 진로선택 | 세포와 물질대사, 생물의 유전 |

### 생물의 유전 (진로선택) — ★**2학기에 실제로 가르치는 과목**
성취기준 17개(12유전01~03)·교과서 차례·용어 표기·PDF 다루는 법은 **`생물의 유전/_기초파악_교육과정_교과서.md`가 정본**이다.
진도·수행평가는 `생물의 유전/_진도계획_2학기.md`, 활동 제작 계획은 `작업노트/생물의유전_모의실험_계획.md`.
아래 생명과학 표는 **1학기 과목**의 것이니 섞지 말 것(코드 접두사가 `12생과` / `12유전`으로 다르다).

### 생명과학 (일반선택) 성취기준 요약 — 1학기 과목(끝남). 세특 문장·위계 앵커가 이 표를 쓴다
- 참고 파일: `수업자료/study-materials/2022 science curriculum_utf8.txt` (UTF-8 · `수업자료/worksheets/` 에도 같은 파일)
  ★2026-08-08 경로 정정 — 예전엔 루트에 있다고 적혀 있었으나 루트에는 없다.
- 생명과학 섹션 시작: 2563번째 줄

★2026-07-25 교육과정 원문(위 파일 2630~2684행) 대조로 **코드별 내용 정정**. 이전 요약표는 01·02 영역이 한 칸씩 밀려 있었고 **혈액형 판정(02-06)이 누락**돼 있었다. 위계 항목 앵커·문장 생성이 이 표를 쓰므로 그대로 두면 학생 문장에 틀린 성취기준이 복제된다.

| 코드 | 내용 (원문 축약) |
|------|------------------|
| **12생과01-01** | 생물 및 생명과학의 특성 이해 · 성과의 협력적 소통 |
| **12생과01-02** | 세포~생태계 생명 시스템 구성 단계 → 체계적 설명 자료 제작 |
| **12생과01-03** | 물질대사의 에너지 전환 → 생명 활동 에너지 사용 추론 |
| **12생과01-04** | 소화·순환·호흡·배설의 기관계 통합 작용 추론 |
| **12생과01-05** | 물질대사 관련 **질병 조사 방법 고안·수행**, 대사성 질환 예방 토의 |
| **12생과01-06** | **생태계 구조** · 물질 순환 · 에너지 흐름 설명 |
| **12생과01-07** | **개체군과 군집**의 특성 · 상호작용 조사·발표 |
| **12생과02-01** | 신경 세포 구조·기능, 전도 과정 모식도 표현 |
| **12생과02-02** | 시냅스 신호 전달, 약물 영향 사례 조사·발표 |
| **12생과02-03** | 사람 신경계 구조·기능, 중추/말초 설명 |
| **12생과02-04** | **내분비계·신경계 상호작용 → 항상성 유지** 추론 (04·05 아님, 한 코드) |
| **12생과02-05** | 병원체의 종류·특징, **선천·후천 면역** 구분 설명 |
| **12생과02-06** | 항원 항체 반응 특이성, **응집 반응으로 혈액형 판정** ← 혈액형 수행평가의 성취기준 |
| **12생과02-07** | **백신**의 종류·작용 원리 조사, 협력적 소통 |
| **12생과03-01** | 염색체 구조와 DNA·유전자의 관계 설명 |
| **12생과03-02** | 생식세포 형성 ↔ 체세포분열 비교, 연속성·다양성 추론 |
| **12생과03-03** | **생물 진화**의 원리와 연구 사례 조사·협력적 소통 ← 진화 논설문의 성취기준 |
| **12생과03-04** | 생물 분류 체계와 분류군의 차이 설명 |
| **12생과03-05** | 동물·식물 분류군 특징(문 수준), 유연관계 계통수 표현 |

---

## 학습지 HTML 양식

폴더에 두 가지 디자인 계열이 있으며, **신규 제작 시 양식 B(파랑 계열)를 기본으로 사용**한다.

### 양식 A — 틸/초록 계열 (구형, 단순)
대표 파일: `worksheets/01_신경세포_구조와기능_학습지.html`
- 단일 HTML, `@page` CSS로 A4 인쇄, 외부 폰트 없음, 단일 페이지
- 색상 토큰: `--deep:#276e68 --main:#3d9890 --soft:#5aada8 --tint:#e5f4f2 --pale:#f2faf9`
- 빈칸 `.blank`: 인라인 블록, 배경 #e8f5f3, bottom border 1.5px #276e68

### 양식 B — 파랑/초록 계열 (신형, 다중 페이지) ← 기본 사용
대표 파일: `worksheets/2-1_신경신호의전달_학습지_v2.html`, `worksheets/2-2_사람의신경계_학습지.html`

#### 구조
- `body` 배경 #d0d0d0 (용지 배경 연출)
- **`.page` div** = A4 (210mm × 297mm, box-shadow)
- 여러 `.page`를 세로 나열 → 다중 페이지
- 외부 폰트: Noto Sans KR (Google Fonts)

#### 색상 토큰
```css
--blue-pastel: #C5E3F7
--green-pastel: #C3E9CC
--blue-dark:   #1255A0
--green-dark:  #2A7A38
--red-accent:  #C0392B
--amber:       #E67E22
--text-main:   #1A1A1A
--text-light:  #555
--line:        #BBBBBB
```

#### 페이지 레이아웃
```
.page
  ├── .page-header  (과목명 + breadcrumb | 학번 + 이름)
  ├── .page-content (섹션들)
  └── .page-footer  (.page-badge — 페이지 번호/구분)
```

#### 헤더
- 좌: `.subject` (13pt, bold, blue-dark) + `.breadcrumb` (7.5pt, 단원 경로)
- 우: 학번/이름 (`.uline` — 90px 밑줄)
- 하단: 2.5px blue-dark 구분선

#### 주요 컴포넌트
- **섹션 제목** `.section-title`: left border 4px blue-dark, 배경 blue-pastel; `.green` 변형 있음
- **빈칸**: `.b`(72px) `.b.sm`(50px) `.b.lg`(108px) `.b.xl`(136px) `.b.xs`(32px) — bottom border 1.8px
- **정보 박스** `.info-box`: 테두리 #BBBBBB, 배경 #FAFAFA, line-height 2.1~2.3
- **표** `.data-table`: thead 배경 blue-pastel (`.g`는 green-pastel), 테두리 #BBBBBB
- **흐름도** `.flow-chart` / `.flow-box` / `.flow-arrow`: flex, blue-dark 색상
- **인라인 SVG**: 번호 원형(fill blue/green-dark), `<image>` 태그로 외부 이미지 참조 가능

#### 인쇄 CSS
```css
@media print {
  body { background:none; padding:0; gap:0; }
  .page { box-shadow:none; page-break-after:always; -webkit-print-color-adjust:exact; }
  .page:last-child { page-break-after:avoid; }
}
```

#### 이미지 오버레이 패턴
```html
<!-- .img-wrap: relative 컨테이너 / .overlay: absolute SVG (viewBox = 원본 픽셀 크기) -->
<div class="img-wrap">
  <img src="pdf_images/파일명.png" style="width:100%">
  <svg class="overlay" viewBox="0 0 885 821" xmlns="http://www.w3.org/2000/svg">
    <!-- origin dot → callout line → numbered circle -->
    <circle cx="260" cy="310" r="6" fill="#1255A0" stroke="white" stroke-width="2"/>
    <line x1="260" y1="310" x2="88" y2="152" stroke="#1255A0" stroke-width="2" stroke-dasharray="5,3"/>
    <circle cx="72" cy="136" r="20" fill="#1255A0"/>
    <text x="72" y="143" text-anchor="middle" font-size="20" fill="white" font-weight="bold">①</text>
  </svg>
</div>
```

#### 정답지 변환 규칙
- 빈칸 `<span class="b ..."></span>` → `<span class="ans">정답텍스트</span>`
- `.ans` CSS: `color:#C0392B; font-weight:700; border-bottom:1.8px solid #C0392B; display:inline-block; padding:0 3px;`
- 헤더 border-bottom 색: blue-dark → red-accent
- 학번/이름 영역 → `【 정 답 지 】` 배지 (red border, red text)
- subject 라벨: "생명과학 — 정답지"

---

## 템플릿 및 유틸리티 파일

| 파일 | 설명 |
|------|------|
| `worksheets/_template.html` | 양식 B 순수 빈 템플릿. 수정 위치 `<!-- ◀ -->` 표시. 섹션 제목 4색 변형, 2페이지·SVG 영역 주석 처리 |
| `worksheets/extract_pdf_images.py` | PyMuPDF 기반 PDF 이미지 추출 스크립트 |

### PDF 이미지 활용 루틴
교과서 이미지를 직접 활용하는 것이 SVG 직접 제작보다 효율적. PDF가 제공될 때마다 실행.

```bash
# Run inside the worksheets folder
python extract_pdf_images.py <PDF경로> pdf_images_<단원명>
```

출력된 파일명 목록으로 어떤 이미지가 있는지 파악한 후, 아래 3가지 패턴 중 선택:
- **패턴 1**: 이미지 단독 + `.img-caption` 캡션
- **패턴 2**: 이미지 위에 SVG 번호 레이블 오버레이 (`.img-wrap` + `.overlay`)
- **패턴 3**: 이미지 + 옆에 빈칸 채우기 (2열 flex)

---

## 완성된 학습지 목록

### 파일명 규칙 (2-3부터 적용)
- 형식: `소단원번호_제목_학습지.html` (예: `2-3_내분비계와항상성유지_학습지.html`)
- **쪽번호는 전체 학습지 통합 기준으로 연속 부여**

| 파일 | 내용 | 쪽 | 상태 |
|------|------|----|------|
| `worksheets/2-1_신경신호의전달_학습지_v2.html` | 2-1 신경신호의 전달 — 학생용 | 1~2 | 완성 |
| `worksheets/2-1_신경신호의전달_정답지_v2.html` | 2-1 신경신호의 전달 — 정답지 | 1~2 | 완성 |
| `worksheets/2-2_사람의신경계_학습지.html` | 2-2 사람의 신경계 — 학생용 | 3~4 | 완성 |
| `worksheets/2-2_사람의신경계_정답지.html` | 2-2 사람의 신경계 — 정답지 | 3~4 | 완성 |
| `worksheets/2-3_내분비계와항상성유지_학습지.html` | 2-3 내분비계와 항상성 유지 — 학생용 | 5~6 | 완성 |
| `worksheets/2-3_내분비계와항상성유지_정답지.html` | 2-3 내분비계와 항상성 유지 — 정답지 | 5~6 | 완성 |

### 신경신호의전달 v2 구성 메모
- **Page 1 A섹션**: 뉴런 구조 3-box flow + Na⁺/K⁺ 분포 박스 + 막전위 용어
- **Page 1 B섹션**: 4단계 활동전위 박스 + 이온채널 상태 표 + SVG 그래프 + 실무율
- **Page 2 C섹션**: 민말이집(`p05_img5`) + 말이집(`p07_img2`) 이미지 + 오버레이 + 비교표
- **Page 2 D섹션**: 시냅스 이미지(`p07_img1`, 885×821px) + 오버레이(①②③④) + 전달 과정 5단계
- 이미지 폴더: `worksheets/pdf_images_신경신호/`

### 사람의신경계 구성 메모
- **Page 1 A섹션**: 신경계 구성 계층도 + `p07_img1`(뇌+척수 측면도) 이미지
- **Page 1 B섹션**: 뇌 시상단면 `p03_img3`(372×363px) + SVG 오버레이(①대뇌 ②사이뇌 ③소뇌 ④⑤⑥뇌줄기) + 기능 표
- **Page 2 C섹션**: 척수 구조 `p05_img1`(650×443px) 이미지 + 빈칸레이블(①겉질/백색질 ②속질/회색질 ③후근 ④전근) + 척수반사 흐름도·비교표
- **Page 2 D섹션**: 신경절 위치·뉴런 길이 비교 SVG 다이어그램(체성/교감/부교감 3행) + 비교표
  - 교감: 신경절 척추 근처 → 이전짧음/이후긺, ACh→노르에피네프린
  - 부교감: 신경절 효과기 근처 → 이전긺/이후짧음, ACh→ACh
- **Page 2 E섹션**: 길항작용 표(6기관) + 핵심정리·예외 info-box
- 이미지 폴더: `worksheets/pdf_images_신경계/` (2-2. 사람의 신경계.pdf 에서 추출)

---

## 작업 원칙

### 언어
- 기본 소통 언어: 한국어
- 수업자료 내 용어: 한국 교육과정 기준 용어 사용 (필요시 영문 병기)

### ★학생용 자료의 말투 — 시험지 문체 (2026-08-17 지시)
**학생이 읽는 모든 문구는 친근체가 아니라 시험 문제 문체로 쓴다.** 반말/존댓말의 문제가 아니라 거리감의 문제다.
- 지시문 = `~하시오` · 설명·판정 = `-이다/-한다` · 선택형 발문 = 물음표로 끝냄
- 오답 피드백은 "옳지 않다." / "반례가 될 수 없다."로 시작하고 이유를 잇는다
- 금지: 좋아 / 맞아 / ~해 보자 / 찾았어 / 거야 / 네가 · 우리가 같은 2인칭 호칭
- **이모지는 금지가 아니다** (2026-08-18 교사 정정: "추임새 이모지는 넣어도 돼. 말투의 문제야").
  🤔 🎉 ✅ 👆 💡 📌 ✏️ 🔎 다 써도 된다 — **문체만 시험지 문체로 유지하면 된다.**
  ※2026-08-17에 한 번 이모지까지 걷어냈다가 되돌린 이력이 있다. 다시 걷어내지 말 것.
- 교사에게 뜨는 확인 대화상자는 `~하시겠습니까?`
> 회귀 검사 예시: `생물의 유전/활동/_test_가계도분석.js`의 **[13] 말투 점검**(금지어·지시문 형태를 기계 검사).
> 새 활동을 만들 때 이 절을 복제해 쓸 것.

### 자료 형식
- 특정 형식을 고정하지 않고 요청 시 용도에 맞는 형식으로 제작
- (예: 인쇄용 PDF 구성, 인터랙티브 HTML, 학습지, 시험 문항 등)

### ★큰 일을 시작할 때는 plan mode 를 권할 것 (2026-09-10 지시)
**되돌리기 비용이 큰 일**을 지시받으면, 착수 전에 교사에게 **한 줄로** 권한다.

> 「이 일은 plan mode 로 하시는 게 낫습니다. **Shift+Tab 을 두 번** 누르시면 됩니다.」

- 권하는 자리: **새 웹활동 · 새 앱 · 자료 구조를 바꾸는 일 · 학생에게 나가는 것을 새로 만드는 일**
- 권하지 않는 자리: 고치기 · 배포 · 점검 · 물어본 것에 답하기 — **매번 권하면 잔소리가 된다**
- **묻고 나서 기다리지 않는다.** 교사가 그냥 하라고 하거나 대답이 없으면 그대로 진행한다
- 까닭: 이 PC 는 학교 공유 계정이라 claude.ai 를 쓰지 않는다
  (`no-session-mirroring`). 「대화형에서 기획하고 코드에서 구현」의 자리를 plan mode 가 대신한다

---

## 이 폴더에 걸어 둔 도구 (2026-09-10 신설 · `.claude/` 는 gitignore 라 USB 로만 따라다닌다)

### 슬래시 명령 — `.claude/commands/`
| 명령 | 언제 |
|---|---|
| `/새활동 <소단원 주제>` | 새 인터랙티브 웹 활동 착수 (제작 표준 → plan mode 권유 → 제작) |
| `/검사 <활동이름>` | 회귀 검사 실행 + **변이 확인** · 검사를 새로 쓸 때의 규율 |
| `/활동배포 <HTML> <앱이름>` | 별도 레포 push → Pages → **허브 카드 등록**까지 |
| `/퀴즈추가 <소단원>` | 복습 퀴즈 문항 이어 붙이기 (`_퀴즈올리기.js` 한 길) |
| `/세션마감` | **재개지점 정제** + 실명 점검 + 미커밋 짚기 |

각 명령은 **절차를 복제하지 않고 정본 노트를 가리킨다.** 규칙이 바뀌면 노트만 고친다.

### 커밋 전 자동 점검 — `.claude/_커밋전점검.py` (PreToolUse 훅)
`git commit` 이 들어오면 **자동으로** 돌고, 걸리면 커밋을 막는다.
① 학생 실명·학번(`_노트_실명점검.py`) ② 시험 자료 경로(중간고사·기말고사·지필·이원목적·시험지·출제)
③ 유니브 폴더. **사람이 기억해서 돌리던 것을 기계로 옮긴 것이다** — 2026-09-08 사고가 계기.

---

## 제작 노하우

### CSS — 색상 변형 클래스 누락 주의
- `.section-title.amber`, `th.r`, `th.y` 등은 개별 파일 CSS에 없으면 기본 파란색으로 fallback됨
- 새 색상 변형 사용 시 `<style>` 블록에 반드시 함께 추가

```css
.section-title.amber { border-left-color:var(--amber); background:#FDE9C9; }
.data-table th.r { background:#F7C5C5; }   /* 붉은 계열 */
.data-table th.y { background:#FDE9C9; }   /* 노란 계열 */
```

### CSS — 섹션 색계열 통일
- 섹션 제목 배경색 = 표 헤더 배경색 값으로 맞출 것
- 중요 행 강조 테두리(imp 클래스)도 해당 섹션 색계열에 맞게 설정

### CSS — 표 셀 정렬
- 모든 td: `text-align:center` + `vertical-align:middle` 필수
- `vertical-align:top` 사용 금지 (`.data-table.detail td` 포함)
- `td.left` 클래스 사용 금지

### CSS — 셀 텍스트 줄바꿈 방지
- 소량만 다음 줄로 넘어갈 경우, 해당 셀에만 인라인 `style="font-size:Xpt"` 적용
- 표 전체 font-size를 줄이지 말 것

### 2022 개정 교육과정 용어 변경

| 구 용어 | 신 용어 |
|--------|--------|
| 티록신 | 타이록신 |
| 글리코젠 | 글라이코젠 |
| 표적 기관 | 표적 세포 |

### 과학적 정확성 체크포인트
- **골격근 떨림(전율)**: 자율신경이 아닌 **체성신경** 지배 → 자율신경 행에 포함 금지
- **뇌하수체 후엽 호르몬(ADH, 옥시토신)**: 시상하부에서 합성, 후엽에서 분비 → 명시 필요
- 호르몬 분비 기관과 합성 기관이 다른 경우 학습지에 별도 표기

### 빈칸 설계 원칙
- 중요도에 따라 선택적으로 뚫기 — 모든 내용을 빈칸으로 만들 필요 없음
- 빈칸 크기는 정답 길이에 맞게 (xs: 32px / sm: 50px / lg: 108px)
- 학생이 굳이 적을 필요 없는 자명한 내용은 미리 채워두기
- 중요 약어(TRH, TSH, ADH 등)는 빈칸으로 강조 가능

### 표 구성 원칙
- 동일 기관이 여러 행일 경우 `rowspan`으로 합치기
- 중요 행 강조: `.imp`(단일행) / `.imp-s`+`.imp-e`(다중행 그룹) 클래스 활용
- 표 헤더 색상으로 열의 성격 구분 (예: 상승/자극=붉은 계열, 하강/억제=기본)

## 인터랙티브 웹 활동 핵심 규칙
- 단일 `.html` 파일로 완결 (외부 파일 없음)
- 태블릿 터치 지원 (iOS Safari / Android Chrome)
- 버튼 영역 최소 44×44px, 폰트 최소 16px
- "다시 하기" 버튼 필수
- 점수 또는 진행 상황 화면에 표시

> ★**새 웹활동을 만들기 전에 `작업노트/방법_웹활동_제작표준.md` 를 먼저 읽을 것.**
> 화면 뼈대(아래는 💪 더 풀어 보기 + ⑤ 정리하기) · 2단 배치 · 애니메이션 구조 · 시각 부호화 ·
> SVG 함정 · 공통 기계 · 회귀 검사 · 브라우저 실측 · 배포 절차가 **거기가 정본**이다.
> **여기에 목록을 복제하지 않는다**(2벌 관리하다 실제로 어긋났음).

## 진행 중인 작업 — ★여기 적지 않는다

**「다음 할 일」은 `작업노트/_재개지점.md`에만 둔다.** 이 파일에 또 적으면 두 벌이 되어 어긋난다(실제로 어긋났다).

> 2026-08-18 정리: 이 자리에 **2026-04 기준 「다음 작업 후보: 2-4 면역 학습지」** 가 남아 있었다.
> 그 사이 학기가 바뀌어(2학기 = 진로선택 「생물의 유전」) 넉 달째 사실이 아니었다.
> 1학기 생명과학 학습지 3종(2-1·2-2·2-3)이 완성돼 있다는 사실은 위 「완성된 학습지 목록」 표가 정본이다.
