...
- **무결성**: 확정본 빈 본문 0 / 초안 빈 본문 0. 확정본 바이트 92~1097(평균 752). 이상치는 데이터 오류 아님 — 92B(한 학생, 답변이 적어 교사가 짧게 확정), 900B 초과 5건(학번은 gitignore된 데이터 폴더에서 확인).
- **이전 미해결 항목 해소**:
  - 이름 null 12명(학번 목록은 백업 JSON에서 조회 — 노트에 미기재) = **데이터 이상 아님, 단순 미참여 확정.** 전원 pin_hash 없음·답변/제출/초안/확정 전무 → 한 번도 로그인(이름 기입) 안 한 학생. (6/8 메모의 "13명"은 12명이 정확.)
  - 등록했으나 무응답 8명 (학번·이름 명단은 백업 JSON에서 조회 — 노트에 실명 미기재)
